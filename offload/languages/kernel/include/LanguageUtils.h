...
static inline ol_result_t waitOnBlockingStreams(StateTy &State, ThreadStateTy &ThreadState) {
...
/// a blocking stream.
static inline ol_result_t waitOnLegacyDefaultStream(StateTy &State, ThreadStateTy &ThreadState, StreamTy *SourceStream,
  ...
  if (Result != OL_SUCCESS) {
    if (Event) {
      SmallVector<ol_event_handle_t, 1> Events = {Event};
      syncAndDestroyEvents(Events);
    }
    return Result;
  }
