...
public:
  ...
  // Reset the sate to allow reuse of the instrumentor with a different
  // configuration.
  void clear() {
...
PreservedAnalyses InstrumentorPass::run(Module &M, InstrumentationConfig &IConf,
  ...
  // once.
  if (ReadConfig)
    readConfigPathsFile(ConfigPathsFile, ConfigFiles, IIRB.Ctx);
  ...
  do {
    ...
    if (!readConfigFromJSON(IConf, ConfigFile, IIRB.Ctx))
      continue;

    auto AddSuffix = [&](std::string S) {
      return MultipleConfigs ? S + "." + std::to_string(Idx) : S;
    };

    writeConfigToJSON(IConf, AddSuffix(OutputConfigFile), IIRB.Ctx);
