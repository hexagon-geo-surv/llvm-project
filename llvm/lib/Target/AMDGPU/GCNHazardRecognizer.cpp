...
int GCNHazardRecognizer::getWaitStatesSinceVALU(IsHazardFn IsHazard,
                                                int Limit) const {
  // An intervening XDL WMMA holds the matrix pipe for its whole latency, so it
  // covers that many wait states rather than the single one a VALU covers.
  if (isHazardRecognizerMode()) {
    auto GetVALUWaitStates = [this](const MachineInstr &MI) -> unsigned {
      if (TII.isXDLWMMA(MI))
        return TII.getInstrLatency(MI);
  ...
  for (MachineInstr *MI : EmittedVALUInstrs) {
    ...
    if (MI && TII.isXDLWMMA(*MI))
      WaitStates += TII.getInstrLatency(*MI);
    else
      ++WaitStates;
...
//   WMMA_*BF8FP8
//   WMMA_*BF8BF8
//   On gfx1250 the 4-cycle 16x16x64 FP8/BF8 and both-f4
