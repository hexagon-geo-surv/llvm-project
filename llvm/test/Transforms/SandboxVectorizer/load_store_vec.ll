...
; RUN: opt -passes=sandbox-vectorizer -sbvec-vec-reg-bits=1024 -sbvec-allow-non-pow2 -sbvec-passes="seed-collection(enable-diff-types)<load-store-vec>" -use-constant-int-for-fixed-length-splat %s -S | FileCheck %s
...
; Same but with vector ConstantInt
define void @load_store_vec_constants_CI_vector(ptr %ptr) {
...
;
  ...
  %ptr2 = getelementptr i8, ptr %ptr, i32 3
  store i8 0, ptr %ptr0
  store <2 x i8> splat (i8 1), ptr %ptr1
  store <5 x i8> splat (i8 2), ptr %ptr2
  ret void
}

; Same but with vector ConstantFP
define void @load_store_vec_constants_CFP_vector(ptr %ptr) {
...
;
  ...
  %ptr2 = getelementptr float, ptr %ptr, i32 3
  store float 1.0, ptr %ptr0
  store <2 x float> splat (float 2.0), ptr %ptr1
  store <5 x float> splat (float 3.0), ptr %ptr2
  ret void
}
