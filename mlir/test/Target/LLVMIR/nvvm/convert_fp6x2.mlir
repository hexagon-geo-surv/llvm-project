...
llvm.func @convert_f6x2_to_bf16x2_e2m3(%src : vector<2xi8>, %scale_factor : i16) {
  ...
  %res3 = nvvm.convert.f6x2.to.bf16x2 %src  sat = <satfinite> : vector<2xi8> (f6E2M3FN) -> vector<2xbf16>
  ...
  %res4 = nvvm.convert.f6x2.to.bf16x2 %src  sat = <satfinite> relu = true : vector<2xi8> (f6E2M3FN) -> vector<2xbf16>
...
llvm.func @convert_f6x2_to_bf16x2_e3m2(%src : vector<2xi8>, %scale_factor : i16) {
  ...
  %res3 = nvvm.convert.f6x2.to.bf16x2 %src  sat = <satfinite> : vector<2xi8> (f6E3M2FN) -> vector<2xbf16>
  // CHECK: %[[res4:.*]] = bitcast <2 x i8> %{{.*}} to i16
  // CHECK-NEXT: %{{.*}} = call <2 x bfloat> @llvm.nvvm.e3m2x2.to.bf16x2.rn.relu.satfinite.scale.n2.ue8m0(i16 %[[res4]], i16 32639)
  %res4 = nvvm.convert.f6x2.to.bf16x2 %src  sat = <satfinite> relu = true : vector<2xi8> (f6E3M2FN) -> vector<2xbf16>
  // CHECK: %[[res5:.*]] = bitcast <2 x i8> %{{.*}} to i16
